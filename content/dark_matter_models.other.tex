...
\end{equation} 
%
where $\alpha$ describes the transition, $\beta$ the outer slope and $\gamma$ the inner slope. \add{Following Newtonian dynamics --- that is fully sufficient on halo scales --- then the velocity is given by \begin{equation}
...
\add{In \cref{fig:profile-illustration-mep} we illustrate the typical morphology of common DM halo models used here for the SPARC galaxies. For a better comparison the plots are normalized with respect to the halo located at the velocity maximum on halo scales.}

%% -- DC14 --
Based on the general ($\alpha, \beta, \gamma$)-model, \citet{2014MNRAS.441.2986D} \add{modelled CDM halos including baryonic feedback mechanisms in galaxy formation. They} found that the three parameters ($\alpha, \beta, \gamma$) are related through 
\begin{align}
...
\end{align} 
%
where $X = \log_{10}(M_*/M_\mathrm{halo})$ describes the stellar-to-dark matter ratio. \add{For the circular velocity \cref{eqn:circular-velocity} the enclosed mass \cref{eqn:enclosed-mass} is given by a} hypergeometric function
%
\begin{equation}
...
%% -- NFW --
Alternatively, for $\alpha = 1$, $\beta = 3$ and $\gamma = 1$ the ($\alpha, \beta, \gamma$)-model reduces to the NFW model \citep{1996ApJ...462..563N,1997ApJ...490..493N} as obtained from early DM-only N-body simulations. \add{This DM model develops cuspy halos of the following type}
%
\begin{equation}
...
\end{equation} 
%
\add{For the circular velocity \cref{eqn:circular-velocity} the enclosed mass \cref{eqn:enclosed-mass} is given by}
%
\begin{multline}
...
\end{multline} 
%
\add{With $M_0 \approx M(R_N)$ being the mass scale originally interpreted as the core mass of the halo \citep[e.g.][]{2000ApJ...537L...9S} while we obtain the relation $M_N = 8 M_0$. Further, the density scale $\rho_N$ describes the central density $\rho_0$ and the length scale $R_{\rm N}$ can be identified with the Burkert radius $r_{\rm B}$ fulfilling the condition $\rho(r_{\rm B}) = \rho_0/4$.}
...
%Another common DM halo model is the pseudo-isothermal sphere model which is a simplified model of the isothermal sphere developing a cored halo \citep{Binney2008}. Note that the isothermal sphere is a limiting case of the fermionic MEPP model with $\beta_0 \to 0$, $\theta_0 \ll -1$ and $W_0 \to \infty$. It is also a special case of the Hernquist model with $\alpha = 2$, $\beta=2$ and $\gamma = 0$. The density profile is described by \begin{equation}
%     \frac{\rho(r)}{\rho_\mathrm{N}} = \qbracket{1 + \bracket{\frac{r}{R_\mathrm{N}}}^{2}}^{-1}
%\end{equation} The enclosed mass of such a profile is given by \begin{equation}
%     \frac{M(r)}{M_{\rm N}} = \frac{r}{R_{\rm N}} - \arctan(r/R_{\rm N})
%\end{equation} The circular velocity is then given by \cref{eqn:circular-velocity}.